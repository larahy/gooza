...
export default class AllPlacecasts {
  ...
  add ({placecast}) {
    ...
    return knex("placecasts").insert({
      ...
      pitch: placecast.pitch,
      heading: placecast.heading,
      zoom: placecast.zoom,
      geom: st.geomFromText(`Point(${long} ${lat})`, 4326),
      user_id: placecast.user_id
    }, ['id', 'title', 'subtitle',
        's3_audio_filename','s3_photo_filename',
        st.asGeoJSON('geom'), 'user_id',
        'created_at', 'published', 'pitch', 'heading', 'zoom'
    ])
      .then(placecast => {
  ...
  findAll (params) {
    ...
    return knex
        .select('id', 'title', 'subtitle',
            's3_audio_filename', 'published',
            's3_photo_filename', st.asGeoJSON('geom'),
            'user_id', 'created_at', 'pitch', 'heading', 'zoom' )
        .from('placecasts')
        .then(results => {
